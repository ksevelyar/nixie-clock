        ...
        + embedded_hal::blocking::i2c::WriteRead<Error = E>,
{
    ...
    pub fn get_rds_time(&mut self) -> Result<(u8, u8, i16), Error<E>> {
        let blk_b = self.read_register(Register::RDA5807M_REG_RDSB)?;
        let blk_c = self.read_register(Register::RDA5807M_REG_RDSC)?;
        let blk_d = self.read_register(Register::RDA5807M_REG_RDSD)?;

        // Decode group type from Block B
        let group_type = (blk_b >> 11) & 0x07;

        // Only process Group 4A (Group Type 4, Version A)
        if group_type != 4 {
            return Err(Error::InvalidGroupType);
        }

        // Extract raw time information
        let raw_minute = blk_d & 0x3F; // Bits 0-5 of Block D
        let raw_hour = (blk_c >> 8) & 0x1F; // Bits 6-10 of Block D
        let raw_offset = (blk_d >> 6) & 0x03;

        // Validate raw time
        //if raw_minute >= 60 || raw_hour >= 24 {
        //    return Err(Error::InvalidTime);
        //}

        Ok((raw_hour as u8, raw_minute as u8, raw_offset as i16))
    }
